...
이 디렉터리는 MTK Easymesh 스택에서 **유·무선 단말 연결/이탈**, **Agent 온보딩 후 `br-lan` IP 할당**, **WPS 온보딩(Controller/Agent)**, **Driver↔Daemon 이벤트 전달**을 정리한 문서를 둡니다.
...
| [WPS_ONBOARDING_PROCESS.md](./WPS_ONBOARDING_PROCESS.md) | WPS 온보딩 전체 흐름(Controller/Agent), `mapd`/`wappd`/`1905daemon` 함수 체인, `hostapd`/`wpa_supplicant` 호출 경로, Mermaid 시퀀스 다이어그램 |

### 추천 열람 순서
1. `WPS_ONBOARDING_PROCESS.md`
2. `AGENT_BRLAN_IP_ASSIGN_FLOW.md`
3. `AGENT_BRLAN_IP_ASSIGN_SEQUENCE_DIAGRAM.md`
4. `유무선-단말-연결-이벤트.md`
5. `컨트롤러-Agent-유선단말-관리.md`
